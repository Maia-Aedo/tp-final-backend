import { BadGatewayException, BadRequestException, Injectable, NotFoundException, UnauthorizedException } from '@nestjs/common'; // lanzan errores HTTP específicos con códigos de estado y mensajes.
...
export class UsuariosService {
  ...
  async findAll(paginator: PaginatorDto) {
    ...
    // dividimos por cantida por pagina
    ...
    const data = await this.prisma.usuario.findMany({
      //en caso de que no existan estos datos, tare todo directamente
    ...
    return {
      data,
      //los metodos dependen de la informacion
      // provista por paginador
